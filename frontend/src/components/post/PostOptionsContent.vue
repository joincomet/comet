  ...
  <v-list>
    ...
    <v-list-item v-if="isMod || isAdmin" @click="togglePin">
      ...
      <v-list-item-content>
        <v-list-item-title style="font-weight: 500">
          {{ post.sticky ? 'Unpin' : 'Pin' }} Post (Mod)
        </v-list-item-title>
      </v-list-item-content>
    </v-list-item>
    ...
    <template v-if="isMod && !post.author.isCurrentUser">
      ...
      <v-list-item @click="banUserFromPlanet">
        ...
        <v-list-item-content>
          <v-list-item-title style="font-weight: 500">
            Ban {{ post.author.username }} from p/{{ post.planet.name }}
            (Mod)
          </v-list-item-title>
        </v-list-item-content>
      </v-list-item>
    ...
    <template v-if="isAdmin && !post.author.isCurrentUser">
      ...
      <v-list-item @click="globalBanAndPurge">
        ...
        <v-list-item-content>
          <v-list-item-title style="font-weight: 500">
            Ban from Comet & purge posts (Admin)
          </v-list-item-title>
        </v-list-item-content>
      </v-list-item>
    </template>
...
export default {
  ...
  methods: {
    ...
    async toggleHide () {
      if (this.post.isHidden) { await this.unhidePost() } else { await this.hidePost() }
    },
    async hidePost () {
      this.$store.dispatch('displaySnackbar', {
    ...
    async togglePin () {
      if (this.post.sticky) { await this.unpinPost() } else { await this.pinPost() }
    },
    async pinPost () {
      this.$store.dispatch('displaySnackbar', {
    ...
    async deletePost () {
      ...
      if (this.post.type === 'TEXT') { this.post.textContent = '<post>[deleted]</post>' }
    },
    async removePost () {
