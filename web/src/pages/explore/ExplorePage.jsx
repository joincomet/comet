...
const NUMBER_OF_SKELETON_INFO_CARDS = 20
...
export default function ExplorePage() {
  ...
  for (let i = 0; i < NUMBER_OF_SKELETON_INFO_CARDS; i++) {
    skeletonInfoCards.push(
      <div className={`relative flex flex-col w-full rounded-lg group dark:bg-gray-800 bg-white animate-pulse`}>
        <div
          className="h-32 rounded-t-lg w-full bg-cover bg-center bg-no-repeat relative dark:bg-gray-700 bg-white"
          
        >
          <div className="absolute left-4 -bottom-3">
            <Avatar
              size={10}
              className="dark:bg-gray-750 rounded-xl ring-4 dark:ring-gray-800 ring-white bg-white"
            />
          </div>
        </div>

        <div className="flex flex-col flex-grow px-4 pt-5 pb-4 h-40">
          <div className="text-lg font-semibold text-secondary h-5 w-3/4 dark:bg-gray-750 rounded-full">
            
          </div>

          <div className="text-13 text-tertiary line-clamp-3 pt-1 h-5 w-1/2 dark:bg-gray-750 rounded-full mt-3">
            
          </div>

          <div className="flex mt-auto text-xs">
            <div className="inline-flex items-center h-5 w-1/2 dark:bg-gray-750 rounded-full">
              
            </div>

            <div className="ml-auto inline-flex items-center  h-5 w-1/4 dark:bg-gray-750 rounded-full">
  ...
  return (
    ...
    >
      ...
      <PageView className={loading ? 'overflow-y-hidden' : 'overflow-y-auto'}>
