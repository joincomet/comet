...
const metascraper = require('util/metascraper')([
...
export const scrapeMetadata = async (targetUrl: string): Promise<LinkEmbed> => {
  ...
  let meta
  try {
    meta = (await metascraper({ html, url })) as LinkEmbed
  } catch {
    return null
  }
